...
process ART_ILLUMINA_PAIRED_END {
    ...
    label 'ART'
    conda 'bioconda::art=2016.06.05=h*_8 bioconda::bbmap=39.01 conda-forge::gsl conda-forge::libcblas conda-forge::libcxx'

    publishDir params.output_dir, mode: "copy", saveAs: {filename -> "fastqs/${prefix_filename}${filename}"}
    ...
    -m ${paired_end_mean_frag_len} --fcov ${depth} --out out_

    bbnorm.sh in=out_1.fq in2=out_2.fq out=art_out_1.fq out2=art_out_2.fq target=50 min=10 threads=$task.cpus
    """
}

process BBNORM_SINGLE_END {
    input:
        tuple val(prefix_filename),
            val(rho),
            val(theta),
    ...
    script:
    """
    bbnorm.sh in=art_out.fq out=bbn_out.fq target=50 min=10 threads=$task.cpus
    """
}

process BBNORM_PAIRED_END {
    input:
        tuple val(prefix_filename),
            val(rho),
            val(theta),
    ...
    script:
    """
    bbnorm.sh in=art_out_1.fq in2=art_out_2.fq out=bbn_out_1.fq out2=bbn_out_2.fq target=50 min=10 threads=$task.cpus
    """
}
...
workflow {
    ...
    params.recom_rates = [0.0] // unscaled r values. rho = 2 . p . N_e . r . tractlen
    params.mutation_rates = [0.0005, 0.0025, 0.005] // unscaled u values. theta = 2 . p . N_e . u
    params.sample_sizes = [50,100,150,200]
